...
// review): UserAccessTokenResult and RefreshTokenResult are plain structs with
...
// integrator's incidental debug log line to leak a live user access token.
//
// The fix (auth.go, UserAccessTokenResult.String / RefreshTokenResult.String)
...
// entire reason GetUserAccessToken/RefreshToken exist is so the caller can
// persist the real values into its own credential store, so over-redacting the
...
func TestUserAccessTokenResult_StringRedactsTokens(t *testing.T) {
	result := &qdmp.UserAccessTokenResult{
	...
	for _, format := range []string{"%v", "%+v"} {
		...
		if strings.Contains(out, sentinelAccessToken) {
			t.Fatalf("fmt.Sprintf(%q, *UserAccessTokenResult) = %q, leaked the raw accessToken", format, out)
		}
		if strings.Contains(out, sentinelRefreshToken) {
			t.Fatalf("fmt.Sprintf(%q, *UserAccessTokenResult) = %q, leaked the raw refreshToken", format, out)
		}
		if !strings.Contains(out, "[REDACTED]") {
			t.Fatalf("fmt.Sprintf(%q, *UserAccessTokenResult) = %q, want it to contain the [REDACTED] placeholder", format, out)
		}
		// Non-secret fields must still be visible.
		if !strings.Contains(out, "1785508950") || !strings.Contains(out, "open-id-1") {
			t.Fatalf("fmt.Sprintf(%q, *UserAccessTokenResult) = %q, want ExpiresAt/OpenID still printed", format, out)
		}
	}
	...
	if strings.Contains(valueOut, sentinelAccessToken) || strings.Contains(valueOut, sentinelRefreshToken) {
		t.Fatalf("fmt.Sprintf(%%v, UserAccessTokenResult value) = %q, leaked a raw token", valueOut)
	}
}

// TestUserAccessTokenResult_GoStringRedactsTokens is a regression test for a
...
func TestUserAccessTokenResult_GoStringRedactsTokens(t *testing.T) {
	result := &qdmp.UserAccessTokenResult{
		AccessToken:  sentinelAccessToken,
		RefreshToken: sentinelRefreshToken,
		ExpiresAt:    "1785508950",
	...
	if strings.Contains(out, sentinelAccessToken) || strings.Contains(out, sentinelRefreshToken) {
		t.Fatalf("fmt.Sprintf(%%#v, *UserAccessTokenResult) = %q, leaked a raw token", out)
	}
	if !strings.Contains(out, "[REDACTED]") {
		t.Fatalf("fmt.Sprintf(%%#v, *UserAccessTokenResult) = %q, want the [REDACTED] placeholder", out)
	}
}
...
// TestUserAccessTokenResult_SlogRedactsTokens is a regression test for a
...
// slog.Info("credential", "value", result) would still emit the raw
...
// by slog itself.
func TestUserAccessTokenResult_SlogRedactsTokens(t *testing.T) {
	result := &qdmp.UserAccessTokenResult{
		AccessToken:  sentinelAccessToken,
		RefreshToken: sentinelRefreshToken,
		ExpiresAt:    "1785508950",
	...
	for _, h := range handlers {
		...
		logger.Info("credential", "value", result)
		out := buf.String()
		if strings.Contains(out, sentinelAccessToken) || strings.Contains(out, sentinelRefreshToken) {
...
// TestUserAccessTokenResult_JSONMarshalNotRedacted is the reverse-direction
// guard: json.Marshal is how the caller persists the real credential into its
// own DB/credential store, so it must keep returning the real values — the
// String()/fmt.Stringer redaction must not leak into encoding/json, which
// only consults json.Marshaler (not fmt.Stringer).
func TestUserAccessTokenResult_JSONMarshalNotRedacted(t *testing.T) {
	result := &qdmp.UserAccessTokenResult{
		AccessToken:  sentinelAccessToken,
		RefreshToken: sentinelRefreshToken,
		ExpiresAt:    "1785508950",
	...
	if err != nil {
		t.Fatalf("json.Marshal(UserAccessTokenResult) returned unexpected error: %v", err)
	}
	out := string(data)
	if !strings.Contains(out, sentinelAccessToken) {
		t.Fatalf("json.Marshal(UserAccessTokenResult) = %q, want it to contain the real accessToken for the caller to persist", out)
	}
	if !strings.Contains(out, sentinelRefreshToken) {
		t.Fatalf("json.Marshal(UserAccessTokenResult) = %q, want it to contain the real refreshToken for the caller to persist", out)
	}
...
// TestAppAccessTokenResult_RedactsTokens is the app-level credential's
// counterpart to the tests above. It gained a refreshToken field (see
// CONTRACT.md §1.1), so it now carries two secrets through exactly the same
// three debug-print paths — %v/%+v (Stringer), %#v (GoStringer) and log/slog
// (LogValuer) — while json.Marshal must still hand back the real values for
// the caller to persist.
func TestAppAccessTokenResult_RedactsTokens(t *testing.T) {
	result := &qdmp.AppAccessTokenResult{
		AccessToken:  sentinelAccessToken,
		ExpiresAt:    "1785508950",
		RefreshToken: sentinelRefreshToken,
		OpenID:       "",
	...
	for _, format := range []string{"%v", "%+v", "%#v"} {
		out := fmt.Sprintf(format, result)
		if strings.Contains(out, sentinelAccessToken) || strings.Contains(out, sentinelRefreshToken) {
			t.Fatalf("fmt.Sprintf(%q, *AppAccessTokenResult) = %q, leaked a raw token", format, out)
		}
		if !strings.Contains(out, "[REDACTED]") {
			t.Fatalf("fmt.Sprintf(%q, *AppAccessTokenResult) = %q, want the [REDACTED] placeholder", format, out)
		}
		if !strings.Contains(out, "1785508950") {
			t.Fatalf("fmt.Sprintf(%q, *AppAccessTokenResult) = %q, want ExpiresAt still printed", format, out)
		}
	}

	valueOut := fmt.Sprintf("%v", *result)
	if strings.Contains(valueOut, sentinelAccessToken) || strings.Contains(valueOut, sentinelRefreshToken) {
		t.Fatalf("fmt.Sprintf(%%v, AppAccessTokenResult value) = %q, leaked a raw token", valueOut)
	}
	...
	for _, h := range []struct {
		...
		var buf bytes.Buffer
		slog.New(h.new(&buf)).Info("credential", "value", result)
		out := buf.String()
		if strings.Contains(out, sentinelAccessToken) || strings.Contains(out, sentinelRefreshToken) {
			t.Fatalf("slog with %s leaked a raw token: %s", h.name, out)
		}
	...
	data, err := json.Marshal(result)
	if err != nil {
		t.Fatalf("json.Marshal(AppAccessTokenResult) returned unexpected error: %v", err)
	}
	out := string(data)
	if !strings.Contains(out, sentinelAccessToken) || !strings.Contains(out, sentinelRefreshToken) {
		t.Fatalf("json.Marshal(AppAccessTokenResult) = %q, want it to contain the real tokens for the caller to persist", out)
	}
}
