...
2. Node.js：`openapi-typescript` 生成 `node/src/generated/schema.d.ts`，再跑 `node/scripts/gen-route-meta.mjs` 同步路由元数据/错误码到 TS。
...
三端测试都是 mock HTTP（Node.js 用 undici `MockAgent`，Java 用 `MockWebServer`，Go 用 `httptest`），不依赖真实网络或真实凭证。

## 覆盖率报告

三端覆盖率工具都是各语言原生自带，纯本地生成，不需要额外依赖或联网：

- **Node.js**：`cd node && npm run test:coverage`（Node.js 内置 test runner 的 `--experimental-test-coverage`），终端直接输出逐文件行/分支/函数覆盖率表格。
...
|---|---|---|---|
| Node.js | `node/package.json` 的 `version` | `node/vX.Y.Z` | `publish-node.yml`：校验版本号一致 → test/build → `npm publish` |
...
- **Node.js**：仓库 secret `NPM_TOKEN`；包名是 scoped 包 `@qdmp/qdmp-server-sdk`，`@qdmp` 这个 npm org 目前还不存在（已查 `registry.npmjs.org` 确认未注册），发布前需要先创建并让 token 归属的账号拥有该 org 的发布权限。
