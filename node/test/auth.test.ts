/**
 * auth 模块测试：app 级 token 缓存 + 单飞刷新 + 300 秒缓冲提前刷新 + code2Session +
 ...
 * - code2Session（AUTHORIZATION_CODE）不进入 app 级缓存
...
void test('auth.getAccessToken: 正常路径用 CLIENT_CREDENTIALS 换取 app 级 token', async () => {
  ...
  assert.equal(token, 'app-token-abc');
...
void test('auth.getAccessToken: 并发调用只触发一次真实换取请求（单飞去重）', async () => {
  const {mockAgent, pool} = createMockAgent();
  ...
  assert.equal(t1, 'app-token-abc');
  assert.equal(t2, 'app-token-abc');
  assert.equal(t3, 'app-token-abc');
  assert.equal(
...
void test('auth.getAccessToken: 缓存命中时不重复发起 HTTP 请求', async () => {
  const {mockAgent, pool} = createMockAgent();
  pool
    .intercept({path: '/auth/v1/token', method: 'POST'})
    .reply(200, businessSuccess(tokenSuccessData()));
  ...
  const first = await client.auth.getAccessToken();
  // 没有再注册第二个拦截器：如果实现没有缓存、又发起了第二次 HTTP 请求，
  // disableNetConnect 会让该请求报错，下面这行就会抛出。
  const second = await client.auth.getAccessToken();

  assert.equal(first, 'app-token-abc');
  assert.equal(second, 'app-token-abc');
});

void test('auth.getAccessToken: 距真正过期还有 300 秒缓冲区间内即主动刷新（不是等到真正过期）', async t => {
...
void test('auth.getAccessToken: 异步 TokenStore 存在"回读延迟"时，紧随其后的调用命中进程内缓存而不重复换取', async () => {
  const {mockAgent, pool} = createMockAgent();
...
void test('auth.getAccessToken: 并发调用者的 tokenStore.get() 在另一调用者的单飞刷新已完成之后才 resolve（且值仍是刷新前的旧快照）时，不应触发第二次冗余换取', async () => {
  const {mockAgent, pool} = createMockAgent();
  ...
  // resolve）。
  const callA = client.auth.getAccessToken();
  const callB = client.auth.getAccessToken();
...
void test('auth.getAccessToken: HTTP 200 但业务 code !== "0" 判定为失败，不能只信 HTTP 状态码', async () => {
  const {mockAgent, pool} = createMockAgent();
  pool
    .intercept({path: '/auth/v1/token', method: 'POST'})
...
void test('auth.getAccessToken: 换取失败时错误信息不泄露 appSecret 原文', async () => {
...
void test('auth.code2Session: AUTHORIZATION_CODE 正常换取用户级 token，且不进入 app 级缓存', async () => {
  const {mockAgent, pool} = createMockAgent();
  let capturedBody: unknown;
  pool
    .intercept({
  ...
  const session = await client.auth.code2Session('login-code-123');

  assert.equal(session.accessToken, 'user-token-xyz');
  assert.equal(session.refreshToken, 'refresh-xyz');
  assert.equal(session.openId, 'openid-abc');
  ...
  // 证明 code2Session 拿到的用户级 token 没有被误当成 app 级 token 缓存复用。
  pool
    .intercept({path: '/auth/v1/token', method: 'POST'})
    .reply(
      200,
  ...
  const appToken = await client.auth.getAccessToken();
  assert.equal(appToken, 'app-token-fresh');
});
...
void test('auth.refreshToken: 空字符串 refreshToken 本地直接报错，不发起任何 HTTP 请求（与 code2Session 对 code 的校验保持一致）', async () => {
  const {mockAgent, pool} = createMockAgent();
  pool
